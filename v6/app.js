...
function renderPricing(data) {
  ...
  cards.forEach(card => {
    ...
    const btnEl = card.querySelector('.price-btn');
    if (btnEl) btnEl.textContent = t(sec.order, currentLang);
...
/* ══ CALCULATOR → WHATSAPP ══ */
function sendCalcToWhatsApp(e) {
  e.preventDefault();
  const selected = [];
  document.querySelectorAll('#calcGrid .calc-item.selected label span').forEach(el => {
    selected.push(el.textContent.trim());
  });
  const total = document.getElementById('calcTotal')?.textContent || '0 €';
  let msg;
  if (selected.length === 0) {
    msg = 'Hallo, ich möchte ein Angebot anfordern. Bitte kontaktieren Sie mich.';
  } else {
    msg = 'Hallo, ich interessiere mich für folgende Leistungen:\n' + selected.map(s => '• ' + s).join('\n') + '\nGeschätzte Kosten: ' + total + '\nBitte kontaktieren Sie mich für ein Angebot.';
  }
  window.open('https://wa.me/4915259859931?text=' + encodeURIComponent(msg), '_blank');
}
