...
const ThemeColor = (() => {
  ...
  function activate(id) {
    ...
    // View may not exist yet during init — guard it
  ...
  function init() {
    ...
    // Apply palette to CSS variables immediately (no DOM rendering needed)
    const p0 = PALETTES.find(x => x.id === savedPalette) || PALETTES[0];
    if (savedPalette === 'custom') {
      applyPalette(paletteFromHex(savedCustom));
    } else {
      applyPalette(p0);
    }
    ...
    if (input) {
      ...
      input.addEventListener('input', e => {
        ...
        if (cp) Object.assign(cp, paletteFromHex(e.target.value));
        activate('custom');
      });
    }
  }
...
const Controller = (() => {
  ...
  /* ── NAVIGATION ── handles both desktop (.header-nav) and mobile (.header-nav-row) */
  function initNav() {
    function handleNavClick(e) {
      const btn = e.target.closest('.nav-btn');
      if (!btn) return;
      // Sync active state across ALL nav buttons (desktop + mobile row)
      document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
      document.querySelectorAll('.form-section').forEach(s => s.classList.remove('active'));
      // Activate matching section
      const sectionId = btn.dataset.section;
      document.querySelectorAll(`.nav-btn[data-section="${sectionId}"]`)
    ...
    document.getElementById('header-nav')?.addEventListener('click', handleNavClick);
    document.getElementById('header-nav-mobile')?.addEventListener('click', handleNavClick);
  }
  ...
  function initMobilePreview() {
    ...
    function updateMobileScale() {
      ...
      // 1123px per page × 2 pages + 24px gap between them
      const scaledHeight = (1123 * 2 + 24) * scale;
