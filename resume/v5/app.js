...
const Controller = (() => {
  ...
  function initPdf() {
    document.getElementById('btn-download').addEventListener('click', () => {
      // Use browser's native print dialog → "Save as PDF"
      // This honours @page { size: A4 } and @media print exactly,
      // with no canvas rasterisation or pixel-scaling artefacts.
      window.print();
    });
  }
  ...
  function initScale() {
    function scale() {
      ...
      /* compensate height so scrollbar works correctly */
      const naturalH = wrap.scrollHeight / s;
      wrap.style.marginBottom = `${-(naturalH * (1 - s))}px`;
    }
    scale();
...
const MobilePreview = (() => {
  ...
  function init() {
    ...
    /* PDF button in mobile bar — reuse window.print() */
