 ...
 *   Flexible Panel • Perfect Mobile Preview • Beautiful Templates
 * ╚═══════════════════════════════════════════════════════════════════════════════╝
 */
'use strict';
...
const View = (() => {
  ...
  /* ── DATE VALIDATION ── */
  // Valid: MM/JJJJ (01-12 / 4-digit year), and "heute" only for "to" fields
  function validateDate(value, isToField) {
    if (!value || value.trim() === '') return 'empty';
    const v = value.trim().toLowerCase();
    if (isToField && v === 'heute') return 'valid';
    // Accept MM/YYYY
    const match = /^(\d{2})\/(\d{4})$/.test(value.trim());
  ...
  function initDateValidation(input) {
    ...
    function check() {
      ...
      if (result === 'valid') {
        ...
        input.classList.remove('date-valid');
    ...
    input.addEventListener('input', e => {
      let v = input.value.replace(/[^0-9hHeuteUTE]/g, '');
      // If typing "heute"
      if (isTo && 'heute'.startsWith(v.toLowerCase()) && v.length <= 5 && isNaN(v[0])) {
        // Let them type freely
      } else {
        // Numeric only: auto-insert slash
        ...
        <div class="list-card-body">
          <div class="fgrid">
            <div class="fg fg-date-wrap">
              ...
              <span class="date-hint">Format: MM/JJJJ (z.B. 03/2018)</span>
            </div>
            <div class="fg fg-date-wrap">
              <label>Bis</label>
              <input type="text" data-ef="dateTo" data-date-field="to" value="${esc(e.dateTo)}" placeholder="MM/JJJJ oder heute" maxlength="7" />
              <span class="date-hint">Format: MM/JJJJ oder „heute"</span>
            </div>
            ...
            <div class="fg full"><label>Aufgaben (eine pro Zeile)</label><textarea data-ef="bullets" placeholder="Aufgabe 1&#10;Aufgabe 2">${(e.bullets||[]).join('\n')}</textarea></div>
          </div>
        </div>`;
      c.appendChild(d);
    });
    // Init auto-resize on newly added textareas
        ...
        <div class="fgrid">
          ${fields.hasDate ? `
            <div class="fg fg-date-wrap">
              <label>Von</label>
              <input type="text" data-ef="dateFrom" data-date-field="from" value="${esc(e.dateFrom)}" placeholder="MM/JJJJ" maxlength="7" />
              <span class="date-hint">Format: MM/JJJJ</span>
            </div>
            <div class="fg fg-date-wrap">
              <label>Bis</label>
          ...
          <div class="fg full"><label>Details (eine pro Zeile)</label><textarea data-ef="bullets" placeholder="Details...">${(e.bullets||[]).join('\n')}</textarea></div>
        </div>
      </div>`;
  }
        ...
        <div class="list-card-body">
          <div class="fgrid">
            ...
            <div class="fg"><label>GER-Niveau</label>
              <select data-ef="cefr" style="border:1.5px solid #e5e5e5;padding:9px 12px;font-size:14px;border-radius:10px;background:#fff;font-family:inherit;width:100%;color:#262626;">
...
const Controller = (() => {
  ...
  function initPanelResizer() {
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      // Auto-adjust zoom to fit
      if (window.innerWidth > 768) {
        const previewPanel = document.querySelector('.preview-panel');
        const wrap = document.getElementById('preview-scale-wrap');
        if (previewPanel && wrap) {
  ...
  function initPersonalFields() {
    ...
    // Auto-resize all static textareas (in personal/additional sections)
  ...
  function initPdf() {
    async function generatePDF() {
      ...
      if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg> <span class="btn-text">Wird erstellt...</span>`;
      }
  ...
  function initMobilePreview() {
    ...
    function updateMobileScale() {
      ...
      const scale = Math.min(1, (vw - padding) / 794);
      document.documentElement.style.setProperty('--mobile-scale', scale);
      // Adjust height compensation
      const naturalHeight = 1123 * 2 + 40; // 2 pages + gap
  ...
  function applyTemplate(name) {
    ['cv-root', 'cv-root-mobile'].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      if (name === 'default') el.removeAttribute('data-template');
      else el.setAttribute('data-template', name);
    });
  }
