...
const View = (() => {
  ...
  */
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     STAGE 1 – buildSections(data)
     Returns an ordered array of SectionDescriptor objects. Pure data, no DOM.
     Each descriptor carries everything needed to render and measure itself.
  ══════════════════════════════════════════════════════════════════════════ */
  function buildSections(data) {
    ...
    /* ── Personal Data ── */
    const persFields = [
      { label: tr('cv.birth'),       value: [d.birthDate, d.birthPlace].filter(Boolean).join(', ') },
      { label: tr('cv.nationality'), value: d.nationality   || '' },
      { label: tr('cv.marital'),     value: d.maritalStatus || '' },
    ].filter(f => f.value);
    if (persFields.length) {
      sections.push({ id: 'personal', title: tr('cv.personal'), type: 'keyvalue', items: persFields, isContinued: false });
    }

    /* ── Training / Weiterbildung ── */
    if (data.weiterbildung.length) {
      sections.push({
        id: 'training', title: tr('cv.training'), type: 'entries',
        items: data.weiterbildung.map(w => ({
          dateStr: esc(w.year),
          titleStr: w.title,
          subStr: w.institution,
          bullets: w.bullets,
        })),
        isContinued: false,
      });
    ...
    if (data.experience.length) {
      sections.push({
        id: 'experience', title: tr('cv.experience'), type: 'entries',
        items: data.experience.map(e => ({
          dateStr: `${esc(e.dateFrom)}${e.dateFrom || e.dateTo ? ' – ' : ''}${esc(e.dateTo)}`,
          titleStr: e.position,
          subStr: `${e.company}${e.location ? ', ' + e.location : ''}`,
          bullets: e.bullets,
        })),
        isContinued: false,
      });
    ...
    if (data.studium.length) {
      sections.push({
        id: 'university', title: tr('cv.university'), type: 'entries',
        items: data.studium.map(e => ({
          dateStr: `${esc(e.dateFrom)}${e.dateFrom || e.dateTo ? ' – ' : ''}${esc(e.dateTo)}`,
          titleStr: e.degree,
          subStr: `${e.institution}${e.location ? ', ' + e.location : ''}`,
          bullets: e.bullets,
        })),
        isContinued: false,
      });
    ...
    if (data.schule.length) {
      sections.push({
        id: 'school', title: tr('cv.school'), type: 'entries',
        items: data.schule.map(e => ({
          dateStr: `${esc(e.dateFrom)}${e.dateFrom || e.dateTo ? ' – ' : ''}${esc(e.dateTo)}`,
          titleStr: e.degree,
          subStr: `${e.institution}${e.location ? ', ' + e.location : ''}`,
          bullets: e.bullets,
    ...
    if (data.languages.length) {
      sections.push({
        id: 'languages', title: tr('cv.languages'), type: 'table',
        thead: `<tr><th>${tr('cv.langCol1')}</th><th>${tr('cv.langCol2')}</th><th>${tr('cv.langCol3')}</th></tr>`,
        items: data.languages.map(l => ({
          row: `<tr><td>${esc(l.name)}</td><td>${esc(l.level)}</td><td>${esc(l.cefr)}</td></tr>`,
        })),
        isContinued: false,
      });
    }
    ...
    if (data.skills.length) {
      sections.push({
        id: 'skills', title: tr('cv.skills'), type: 'keyvalue',
        items: data.skills.map(s => ({ label: s.category, value: s.items, isSkill: true })),
        isContinued: false,
      });
    }
    ...
    /* ── Additional ── */
    const addBullets = [
      ai.drivingLicense && `${tr('field.drivingLicense')}: ${ai.drivingLicense}`,
      ai.hobbies        && `${tr('field.hobbies')}: ${ai.hobbies}`,
      ai.volunteering   && `${tr('field.volunteering')}: ${ai.volunteering}`,
      ai.other          && ai.other,
    ].filter(Boolean);
    if (addBullets.length) {
      sections.push({
        id: 'additional', title: tr('cv.additional'), type: 'bullets',
        items: addBullets.map(b => ({ text: b })),
        isContinued: false,
      });
    }
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     SECTION → HTML  (shared by measurement probe and final render)
  ══════════════════════════════════════════════════════════════════════════ */
  function sectionToHTML(sec) {
    const titleBar = sec.isContinued
      ? `<div class="cv-section-title">${esc(sec.title)} <span style="font-weight:400;font-size:8pt;opacity:0.6">(Forts.)</span></div>`
      : `<div class="cv-section-title">${esc(sec.title)}</div>`;

    let innerHTML = '';

    if (sec.type === 'entries') {
      innerHTML = sec.items.map(it => entryRow(it.dateStr, it.titleStr, it.subStr, it.bullets)).join('');
    } else if (sec.type === 'keyvalue') {
      if (sec.id === 'skills') {
        innerHTML = `<div class="cv-skills-rows">${
          sec.items.map(it => `
            <div class="cv-skill-row">
              <span class="cv-skill-cat">${esc(it.label)}</span>
              <span class="cv-skill-val">${esc(it.value)}</span>
            </div>`).join('')
        }</div>`;
      } else {
        innerHTML = `<div class="cv-personal-rows">${
          sec.items.map(it => `
            <div class="cv-personal-row">
              <span class="cv-pd-label">${esc(it.label)}</span>
              <span class="cv-pd-value">${esc(it.value)}</span>
            </div>`).join('')
        }</div>`;
      }
    } else if (sec.type === 'table') {
      // Continuation pages repeat the thead for clarity
      const theadHTML = sec.isContinued
        ? `<thead>${sec.thead}</thead>`
        : `<thead>${sec.thead}</thead>`;
      innerHTML = `<table class="cv-lang-table">${theadHTML}<tbody>${
        sec.items.map(it => it.row).join('')
      }</tbody></table>`;
    } else if (sec.type === 'bullets') {
      innerHTML = bulletsHTML(sec.items.map(it => it.text));
    }

    return `<div class="cv-section">${titleBar}${innerHTML}</div>`;
  }

  /* ══════════════════════════════════════════════════════════════════════════
     MEASUREMENT HELPERS
     All measurement is done at 1:1 scale in a hidden off-screen container.
     No transform:scale is ever used for layout calculations.
  ══════════════════════════════════════════════════════════════════════════ */
  let _probeContainer = null;

  function _getProbeContainer(template) {
    // Reuse a single off-screen container; update its template attribute each call
    if (!_probeContainer) {
      _probeContainer = document.createElement('div');
      _probeContainer.setAttribute('id', 'cv-measure-probe');
      _probeContainer.style.cssText = [
        'position:fixed',
        'top:0',
        'left:-9999px',
        'width:794px',       // exact A4 CSS width
        'visibility:hidden',
        'pointer-events:none',
        'z-index:-1',
        'overflow:visible',
      ].join(';');
      document.body.appendChild(_probeContainer);
    }
    _probeContainer.setAttribute('data-template', template || 'modern');
    return _probeContainer;
  }

  function measurePageChrome(template) {
    // Returns { headerH, footerH, bodyPadTop, bodyPadBottom }
    // by rendering a minimal page structure and reading computed sizes.
    const probe = _getProbeContainer(template);
    probe.innerHTML = `
      <div class="cv-page">
        <div class="cv-page-header cv-measure-hdr" id="probe-hdr" style="visibility:hidden"></div>
        <div class="cv-page-body" id="probe-body" style="min-height:0;padding-bottom:0"></div>
        <div class="cv-page-number" id="probe-foot" style="position:static;visibility:hidden">X</div>
      </div>`;

    // We need to inject a minimal header so templates that use padding-top
    // on .cv-page-header render at their actual height.
    const hdrEl = probe.querySelector('.cv-page-header');
    // fill with same content as pageHeaderHTML but invisible
    hdrEl.innerHTML = `
      <div class="cv-hdr-row">
        <div class="cv-hdr-left">
          <div class="cv-hdr-name">A</div>
        </div>
        <div class="cv-hdr-photo"></div>
      </div>`;

    const bodyEl  = probe.querySelector('.cv-page-body');
    const footEl  = probe.querySelector('.cv-page-number');
    const bodyCS  = window.getComputedStyle(bodyEl);

    const headerH     = hdrEl.offsetHeight;
    const footerH     = 36; // page-number bottom:16 + branding + comfortable gap
    const bodyPadTop  = parseFloat(bodyCS.paddingTop)    || 0;
    const bodyPadBot  = parseFloat(bodyCS.paddingBottom) || 0;

    probe.innerHTML = '';
    return { headerH, footerH, bodyPadTop, bodyPadBot };
  }

  function measureSection(sec, template) {
    // Renders one section into the probe, returns its offsetHeight in px.
    const probe = _getProbeContainer(template);
    const wrapper = document.createElement('div');
    wrapper.className = 'cv-page-body';
    wrapper.style.cssText = 'position:absolute;top:0;left:0;width:714px;padding:0;margin:0'; // 794 - 2×40 padding
    wrapper.innerHTML = sectionToHTML(sec);
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     STAGE 3 – splitLargeSection(section, firstBudget, fullPageHeight)
     Called only when a section is taller than one full content area.
     Splits at item-level boundaries. Returns array of partial sections.
  ══════════════════════════════════════════════════════════════════════════ */
  function splitLargeSection(sec, firstBudget, fullPageHeight, template) {
    // Measure the section title bar height (shared overhead)
    const probe = _getProbeContainer(template);
    const titleEl = document.createElement('div');
    titleEl.innerHTML = `<div class="cv-section"><div class="cv-section-title">${esc(sec.title)}</div></div>`;
    titleEl.style.cssText = 'position:absolute;top:0;left:0;width:714px;padding:0;margin:0';
    probe.appendChild(titleEl);
    const titleH = titleEl.querySelector('.cv-section-title').offsetHeight + 12; // +marginBottom
    probe.removeChild(titleEl);

    const parts = [];
    let currentItems = [];
    let currentBudget = firstBudget - titleH; // first piece has less room (already has title)
    let isContinued = false;

    for (let i = 0; i < sec.items.length; i++) {
      const item = sec.items[i];
      // Build a single-item partial section to measure it
      const singleSec = { ...sec, items: [item], isContinued };
      let itemH = measureSection(singleSec, template) - titleH;
      if (itemH < 0) itemH = 0;

      if (currentItems.length === 0) {
        // First item on a fresh page – always place it even if it slightly overflows
        // (single items that are bigger than a page can't be split further)
        currentItems.push(item);
        currentBudget -= itemH;
      } else if (itemH <= currentBudget) {
        currentItems.push(item);
        currentBudget -= itemH;
      } else {
        // Flush the current accumulation as a part
        parts.push({ ...sec, items: [...currentItems], isContinued });
        // Start fresh page
        isContinued = true;
        currentItems = [item];
        currentBudget = fullPageHeight - titleH - itemH;
      }
    }
    ...
    return parts.length ? parts : [sec];
  }

  /* ══════════════════════════════════════════════════════════════════════════
     STAGE 2 – paginate(sections, contentHeight, template)
     Distributes section descriptors into page buckets.
     Returns Page[][] – an array of pages, each page is an array of sections.
  ══════════════════════════════════════════════════════════════════════════ */
  function paginate(sections, contentHeight, template) {
    const pages  = [[]];
    let remaining = contentHeight;

    for (const sec of sections) {
      const h = measureSection(sec, template);
      ...
      if (h <= remaining) {
        ...
        // Section is bigger than a full page → split it
        const parts = splitLargeSection(sec, remaining, contentHeight, template);
        for (let pi = 0; pi < parts.length; pi++) {
          const part = parts[pi];
          const partH = measureSection(part, template);

          if (pi === 0 && partH <= remaining) {
            // First part fits on current page
            pages[pages.length - 1].push(part);
            remaining -= partH;
          } else {
            // Subsequent parts (or first part that doesn't fit): new page
            pages.push([part]);
            remaining = contentHeight - partH;
          }
        }
      } else {
        // Doesn't fit current page, but fits a full page → move to next page
        pages.push([sec]);
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     STAGE 4 – renderPages(pages, data, totalPages, template, root)
     Writes final HTML into the root element.
  ══════════════════════════════════════════════════════════════════════════ */
  function renderPages(pages, data, template, root) {
    const d = data.personalData;
    const total = pages.length;

    const html = pages.map((pageSections, idx) => {
      const pageNum  = idx + 1;
      const isFirst  = pageNum === 1;
      const bodyHTML = pageSections.map(sec => sectionToHTML(sec)).join('');

      return `
        <div class="cv-page page-${pageNum}${isFirst ? ' page-first' : ''}">
          ${pageHeaderHTML(d, isFirst, template)}
          <div class="cv-page-body">${bodyHTML}</div>
          <div class="cv-page-number">${pageNum} / ${total}</div>
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     renderCV  – public entry point (replaces the old fixed two-page version)
     Pipeline: buildSections → measure chrome → paginate → renderPages
  ══════════════════════════════════════════════════════════════════════════ */
  function renderCV(data, targetId = 'cv-root') {
    ...
    /* 1. Build section descriptors */
    const sections = buildSections(data);

    /* 2. Measure how much vertical space the body area has per page */
    const { headerH, footerH, bodyPadTop, bodyPadBot } = measurePageChrome(template);
    const A4_H = 1123; // px at 96dpi
    const contentHeight = A4_H - headerH - footerH - bodyPadTop - bodyPadBot;

    /* 3. Paginate (calls splitLargeSection internally when needed) */
    const pages = paginate(sections, contentHeight, template);

    /* 4. Render */
    renderPages(pages, data, template, root);
  }
...
const Controller = (() => {
    ...
    async function generatePDF() {
      ...
      try {
        ...
        const opt = {
          ...
          pagebreak:   { mode: ['css'], avoid: ['.cv-section', '.cv-row', '.cv-personal-row', '.cv-skill-row'] }
        };
  ...
  function initMobilePreview() {
    ...
    function updateMobileScale() {
      ...
      // Measure the real content height after render (not hardcoded page count).
      // The scaler is transform:scale'd, so read its natural scrollHeight
      // before scaling and apply that as the outer container height.
