...
const AccentColor = (() => {
  ...
  // Darken a hex color by a factor (0-1 = amount to darken)
  ...
  function applyColor(hex) {
    ...
    root.style.setProperty('--primary-600',  darken(hex, 0.12));
    root.style.setProperty('--primary-700',  darken(hex, 0.22));
    root.style.setProperty('--primary-50',   lighten(hex, 0.92));
    root.style.setProperty('--primary-100',  lighten(hex, 0.82));
    // Also update the CV accent variable used in templates
  ...
  function init() {
    ...
    if (input) {
      ...
      input.addEventListener('input', e => {
        ...
        // Re-render CV to pick up new color in inline styles
        View.renderCV(Model.getData());
        View.renderCV(Model.getData(), 'cv-root-mobile');
      });
    }
