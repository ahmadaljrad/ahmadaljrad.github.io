...
const View = (() => {
  ...
  */
  ...
  function renderCV(data, targetId = 'cv-root') {
    ...
    /* ── BUILD ALL SECTIONS AS HTML BLOCKS ── */
    ...
    // All sections in original display order
    const allSections = [
      persRows      ? `<div class="cv-section">${secTitle(t('cv.personal'))}<div class="cv-personal-rows">${persRows}</div></div>` : '',
      data.weiterbildung.length ? `<div class="cv-section">${secTitle(t('cv.training'))}${data.weiterbildung.map(w => entryRow(esc(w.year), w.title, w.institution, w.bullets)).join('')}</div>` : '',
      data.experience.length    ? `<div class="cv-section">${secTitle(t('cv.experience'))}${data.experience.map(e => entryRow(`${esc(e.dateFrom)}${e.dateFrom||e.dateTo?' – ':''}${esc(e.dateTo)}`, e.position, `${e.company}${e.location?', '+e.location:''}`, e.bullets)).join('')}</div>` : '',
      data.studium.length       ? `<div class="cv-section">${secTitle(t('cv.university'))}${data.studium.map(e => entryRow(`${esc(e.dateFrom)}${e.dateFrom||e.dateTo?' – ':''}${esc(e.dateTo)}`, e.degree, `${e.institution}${e.location?', '+e.location:''}`, e.bullets)).join('')}</div>` : '',
      data.schule.length        ? `<div class="cv-section">${secTitle(t('cv.school'))}${data.schule.map(e => entryRow(`${esc(e.dateFrom)}${e.dateFrom||e.dateTo?' – ':''}${esc(e.dateTo)}`, e.degree, `${e.institution}${e.location?', '+e.location:''}`, e.bullets)).join('')}</div>` : '',
    ...
    /* ── MEASURE SECTION HEIGHTS IN A HIDDEN CONTAINER ── */

    // A4 body usable height: 1123px total
    // Header (page-1 with photo): ~180px | Header (page-2+): ~130px | Footer: 36px | Body padding: 74px
    const PAGE_H          = 1123;
    const HEADER_H_P1     = 185;   // first page header (with photo)
    const HEADER_H_REST   = 135;   // subsequent pages header (no photo)
    const FOOTER_H        = 36;    // page-number + branding
    const BODY_PADDING    = 74;    // cv-page-body top+bottom padding
    const SECTION_GAP     = 20;    // cv-section margin-bottom

    const bodyAvailP1   = PAGE_H - HEADER_H_P1   - FOOTER_H - BODY_PADDING;
    const bodyAvailRest = PAGE_H - HEADER_H_REST  - FOOTER_H - BODY_PADDING;

    // Render all sections into a hidden measuring div
    const measurer = document.createElement('div');
    measurer.style.cssText = [
      'position:absolute', 'top:-99999px', 'left:-99999px',
      `width:714px`,          // 794px A4 - 80px (40px*2) body padding
      'visibility:hidden', 'pointer-events:none',
      'font-family:Inter,Arial,sans-serif', 'font-size:10.5pt', 'line-height:1.5',
    ].join(';');

    // Copy template attribute so CSS cascade applies
    measurer.setAttribute('data-template', currentTemplate);

    allSections.forEach(html => {
      const wrapper = document.createElement('div');
      wrapper.innerHTML = html;
      measurer.appendChild(wrapper.firstElementChild);
    });
    document.body.appendChild(measurer);

    // Collect heights
    const sectionHeights = Array.from(measurer.children).map(el => el.offsetHeight + SECTION_GAP);
    document.body.removeChild(measurer);

    /* ── DISTRIBUTE SECTIONS ACROSS PAGES ── */

    const pages = [];          // array of arrays of section HTML strings
    let currentPage   = [];
    let usedHeight    = 0;
    ...
    allSections.forEach((html, i) => {
      ...
      if (usedHeight + h > avail && currentPage.length > 0) {
    ...
    const pagesHTML = pages.map((sections, idx) => {
      const pageNum   = idx + 1;
      const isFirst   = pageNum === 1;
      const isLast    = pageNum === totalPages;
      const pageClass = isFirst ? 'page-1' : `page-${pageNum}`;

      return `
        <div class="cv-page ${pageClass}">
          ${pageHeaderHTML(d, isFirst, currentTemplate)}
