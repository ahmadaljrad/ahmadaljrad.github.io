...
const AccentColor = (() => {
  ...
  // Professional color palette for CV template control
  const PALETTE = [
  ...
  // Only touch CV template variables — never --primary or any UI variable
  function applyColor(hex) {
    ...
    if (input) { input.value = hex; input.style.outlineColor = hex; }
    // Mark active palette swatch
    document.querySelectorAll('.palette-swatch').forEach(el => {
      el.classList.toggle('active', el.dataset.hex === hex);
    });
    try { localStorage.setItem('cv_accent', hex); } catch(e) {}
  }
  ...
  function init() {
    let saved = '#2563EB';
    try { saved = localStorage.getItem('cv_accent') || '#2563EB'; } catch(e) {}
    ...
    if (trigger && dropdown) {
      trigger.addEventListener('click', e => {
        ...
        if (isOpen) {
          // Position it first (hidden) to measure
          ...
          // Horizontal: prefer right-aligned to trigger, clamp within viewport
          let left = tr.right - dw;
          if (left < 8) left = 8;
          if (left + dw > vw - 8) left = vw - dw - 8;

          // Vertical: below trigger if room, otherwise above
  ...
  return { init, applyColor, getColor: () => {
    try { return localStorage.getItem('cv_accent') || '#2563EB'; } catch(e) { return '#2563EB'; }
  }};
})();
  ...
  */
  ...
  function renderCV(data, targetId = 'cv-root') {
    ...
    const langRows = data.languages.map(l => `
      <tr><td>${esc(l.name)}</td><td>${esc(l.level)}</td><td>${esc(l.cefr)}</td></tr>`).join('');

    const langTable = `<table class="cv-lang-table">
    ...
    const allSections = [
      persRows                  ? `<div class="cv-section" data-sec="personal">${secTitle(t('cv.personal'))}<div class="cv-personal-rows">${persRows}</div></div>` : '',
      data.weiterbildung.length ? `<div class="cv-section" data-sec="training">${secTitle(t('cv.training'))}${data.weiterbildung.map(w => entryRow(esc(w.year), w.title, w.institution, w.bullets)).join('')}</div>` : '',
      data.experience.length    ? `<div class="cv-section" data-sec="experience">${secTitle(t('cv.experience'))}${data.experience.map(e => entryRow(`${esc(e.dateFrom)}${e.dateFrom||e.dateTo?' – ':''}${esc(e.dateTo)}`, e.position, `${e.company}${e.location?', '+e.location:''}`, e.bullets)).join('')}</div>` : '',
      data.studium.length       ? `<div class="cv-section" data-sec="university">${secTitle(t('cv.university'))}${data.studium.map(e => entryRow(`${esc(e.dateFrom)}${e.dateFrom||e.dateTo?' – ':''}${esc(e.dateTo)}`, e.degree, `${e.institution}${e.location?', '+e.location:''}`, e.bullets)).join('')}</div>` : '',
      data.schule.length        ? `<div class="cv-section" data-sec="school">${secTitle(t('cv.school'))}${data.schule.map(e => entryRow(`${esc(e.dateFrom)}${e.dateFrom||e.dateTo?' – ':''}${esc(e.dateTo)}`, e.degree, `${e.institution}${e.location?', '+e.location:''}`, e.bullets)).join('')}</div>` : '',
      data.languages.length     ? `<div class="cv-section" data-sec="languages">${secTitle(t('cv.languages'))}${langTable}</div>` : '',
      data.skills.length        ? `<div class="cv-section" data-sec="skills">${secTitle(t('cv.skills'))}<div class="cv-skills-rows">${skillRows}</div></div>` : '',
      addBullets.length         ? `<div class="cv-section" data-sec="additional">${secTitle(t('cv.additional'))}${bulletsHTML(addBullets)}</div>` : '',
    ].filter(Boolean);

    /* ── STEP 1: RENDER EVERYTHING IN ONE FLAT PAGE TO MEASURE REAL HEIGHTS ── */
    // We render into a real cv-page inside the actual root (inherits all CSS),
    // read the heights, then re-distribute. This guarantees pixel-accurate measurement.

    root.innerHTML = `
      <div class="cv-page page-1" id="_cv_measure_page" style="min-height:0;height:auto;position:absolute;top:-99999px;left:-99999px;visibility:hidden;">
        ${pageHeaderHTML(d, true, currentTemplate)}
        <div class="cv-page-body" id="_cv_measure_body">${allSections.join('')}</div>
      </div>`;

    // Let browser lay out
    const measurePage   = root.querySelector('#_cv_measure_page');
    const measureBody   = root.querySelector('#_cv_measure_body');
    const headerEl      = measurePage.querySelector('.cv-page-header');

    // Real measured heights (px)
    const realHeaderH_P1   = headerEl   ? headerEl.offsetHeight   : 185;
    const realBodyPaddingT = 22;   // from CSS: cv-page-body padding-top
    const realBodyPaddingB = 52;   // from CSS: cv-page-body padding-bottom
    const FOOTER_H         = 36;   // cv-page-number + cv-branding
    const PAGE_H           = 1123; // A4 height in px

    // Available body height per page
    const bodyAvailP1   = PAGE_H - realHeaderH_P1 - realBodyPaddingT - realBodyPaddingB - FOOTER_H;
    // For subsequent pages: header is smaller (no photo shown visually but still rendered for layout)
    // We estimate: subtract photo height difference (~65px) from P1 header
    const bodyAvailRest = bodyAvailP1 + 65;

    // Measure each section's rendered height
    const sectionEls = Array.from(measureBody.querySelectorAll('.cv-section'));
    const sectionHeights = sectionEls.map(el => {
      const style = getComputedStyle(el);
      return el.offsetHeight + parseInt(style.marginBottom || '0', 10);
    });
    ...
    allSections.forEach((html, i) => {
      const h     = sectionHeights[i] || 60;
      const avail = isFirstPage ? bodyAvailP1 : bodyAvailRest;

      if (currentBucket.length > 0 && usedHeight + h > avail) {
        pages.push({ sections: currentBucket, isFirst: isFirstPage });
        currentBucket = [html];
        usedHeight    = h;
        isFirstPage   = false;
      } else {
        currentBucket.push(html);
        usedHeight += h;
      }
    });
    if (currentBucket.length) pages.push({ sections: currentBucket, isFirst: isFirstPage });

    /* ── STEP 3: RENDER FINAL PAGES ── */
    const totalPages = pages.length;

    // Each page gets exact height = A4 ONLY if it's not the last page,
    // so html2pdf sees correct page boundaries without empty pages.
    const pagesHTML = pages.map(({ sections, isFirst }, idx) => {
      const pageNum   = idx + 1;
      const isLast    = pageNum === totalPages;
      const pageClass = pageNum === 1 ? 'page-1' : `page-${pageNum} cv-page-break`;
      // Last page: no forced min-height → no empty space in PDF
      // Other pages: exact A4 height so pagination is clean
      const heightStyle = isLast
        ? 'style="min-height:0;"'
        : `style="min-height:${PAGE_H}px;max-height:${PAGE_H}px;overflow:hidden;"`;

      return `
        <div class="cv-page ${pageClass}" ${heightStyle}>
          ${pageHeaderHTML(d, pageNum === 1, currentTemplate)}
          <div class="cv-page-body">${sections.join('')}</div>
          <div class="cv-page-number">${pageNum} / ${totalPages}</div>
...
const Controller = (() => {
    ...
    async function generatePDF() {
      ...
      try {
        ...
        const opt = {
          ...
          pagebreak:   { mode: ['css-class'], cssClass: 'cv-page-break' }
        };
  ...
  function initMobilePreview() {
    ...
    function updateMobileScale() {
      ...
      // Dynamic height based on actual page count
      ...
      const scaledHeight = (1123 * pageCount + 20 * (pageCount - 1)) * scale;
